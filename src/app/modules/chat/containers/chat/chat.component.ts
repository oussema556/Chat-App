...
export class ChatComponent implements OnInit, OnDestroy{
  ...
  searchTerms: string;
  selectedUserPseudoId: string | null;
  enteredPseudo: string | undefined;
  videosToSend: any;
  searchedUsers: any;
  stopRecordingVocal: BehaviorSubject<any> | undefined;
  ...
  destinationUser: number | undefined;
  imagesToSend: any;
  filesUpload: any;
  nbNotif: number;
  //modals
  openBlockMsgModal: boolean;
  openDeleteConverModal: boolean | undefined;
  ...
  openEmojiModal: boolean | undefined;
  callReceivedModal: boolean;
  callWindow: Window | undefined;
  messageReceivedSound: Howl;
  callReceivedSound: Howl;
  conversationAudios: any;
  @ViewChild('inputMessage', { static: false }) inputMessage: ElementRef | undefined;
  @ViewChild('audioMessage', { static: false }) audioMsg: ElementRef | undefined;


  time: number = 0;
  recordDuration: any;
  saveRecordAudio: boolean | undefined;
  interval: any;
  emoji;
  ...
  constructor(private messengerService: ChatService) {
    ...
    this.messengerService.userHasConnected()
      .subscribe(userid => {
        this.userConversations = this.userConversations.map((conversation:any) => {
          const userIds = conversation.users.map((user: any) => user._id)
          if (userIds.includes(userid))
            conversation.messages = conversation.messages.map((message:any) => {
              return { ...message, reception: true }
            });
          const userIndex = conversation.users.findIndex((user:any) => user._id == userid)
          if (userIndex != -1) {
    ...
    this.messengerService.userHasDisconnected()
      .subscribe((data: any) => {
        this.userConversations.forEach((conversation:any) => {
          const userIndex = conversation.users.findIndex((user:any) => user._id == data.userid)
          if (userIndex != -1) {
    ...
    this.messengerService.seenMessage()
      .subscribe((info: any) => {
        const conversationIndex = this.userConversations.findIndex((conversation:any) => conversation._id == info.conversation)
        this.userConversations[conversationIndex].messages.forEach((message:any) => {
    ...
    this.messengerService.messageReceived()
      .subscribe((message: any) => {
        ...
        if (this.currentConversation && this.currentConversation._id == message.conversation) {
          const msgIndex = this.currentConversation.messages.findIndex((msg:any) => msg._id == message._id)
    ...
    this.messengerService.newMessage()
      .subscribe((newConversation: any) => {
        const conversationIndex = this.userConversations.findIndex((userConversation:any) => userConversation._id == newConversation._id)
  ...
  getDuration(messageId:any) {
    const messageAudioIndex = this.conversationAudios.findIndex((convAudio:any) => convAudio.messageId == messageId)

    return this.conversationAudios[messageAudioIndex].audio._duration;
  }
  transfromDuration(duration:any) {
  ...
  addConversationEmoji(event:any) {
